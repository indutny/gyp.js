...
Condition.prototype.parseOne = function parseOne() {
  this.space();

  let seed = this.parseIdOrLiteral();
...
const KEYWORD_OR_OP = /^(and|del|not\s+in|not|as|or|in|is|==|!=|>|<|>=|<=)/;

Condition.prototype.parseOp = function parseOp() {
  const part = this.str.slice(this.off);
  const match = part.match(KEYWORD_OR_OP);
  if (match === null)
    return false;
