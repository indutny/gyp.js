...
// TODO(indutny): remove framework, bundle support? simplify as much as possible
  ...
  if (sources.length) {
    ...
    if (this.flavor === 'win') {
      gyp.msvsEmulation.verifyMissingSources(
          sources, this.absBuildDir.bind(this), generatorFlags,
          this.gypPathToNinja.bind(this));
      pch = gyp.msvsEmulation.precompiledHeader(
          this.msvsSettings, configName, this.gypPathToNinja.bind(this),
          this.gypPathToUniqueOutput, this.objExt);
    } else {
      pch = gyp.xcodeEmulation.macPrefixHeader(
          this.xcodeSettings, this.gypPathToNinja.bind(this),
    ...
    sources.forEach((source) => {
      ...
      neededVariables.forEach((v) => {
        if (v === 'root') {
          ...
          dirnameExpanded = this.expandSpecial(dirname,
                                               this.baseToBuild.bind(this));
          extraBindings.push([ 'dirname', cygwinMunge(dirnameExpanded) ]);
        } else if (v === 'source') {
          // '$source' is a parameter to the rule action, which means
          // it shouldn't be converted to a Ninja path.  But we don't
          // want $!PRODUCT_DIR in there either.
          const sourceExpanded = this.expandSpecial(
              source,
              this.baseToBuild.bind(this));
...
NinjaWriter.prototype.writeCopies = function writeCopies(copies, prebuild,
                                                         macBundleDepends) {
  let outputs = [];
  let env;
  if (this.xcodeSettings) {
    const extraEnv = this.xcodeSettings.getPerTargetSettings();
    env = this.getToolchainEnv({ additionalSettings: extraEnv });
  } else {
    env = this.getToolchainEnv();
  }
  copies.forEach((copy) => {
    copy['files'].forEach((tpath) => {
      // Normalize the path so trailing slashes don't confuse us.
      tpath = path.normalize(tpath);
      const basename = path.basename(path);
      const src = this.gypPathToNinja(path, env);
      const dst = this.gypPathToNinja(path.join(copy['destination'], basename),
                                      env);
      outputs = outputs.concat(this.ninja.build(dst, 'copy', src, {
        orderOnly: prebuild
      }));
      if (this.isMacBundle) {
        ...
        // input changes.
        if (dst.indexOf(
                this.xcodeSettings.getBundleContentsFolderPath()) === 0) {
          macBundleDepends.push(dst);
        }
      }
    });
  });
...
NinjaWriter.prototype.writeiOSFrameworkHeaders =
    function writeiOSFrameworkHeaders(spec, outputs, prebuild) {
  /* Prebuild steps to generate hmap files and copy headers to destination. */
  const framework = this.computeMacBundleOutput();
  const allSources = spec['sources'];
  const copyHeaders = spec['macFrameworkHeaders'];
  const output = this.gypPathToUniqueOutput('headers.hmap');
  this.xcodeSettings.headerMapPath = output;
  const allHeaders = allSources.filter(x => /\.h$/.test(x)).map((x) => {
    return this.gypPathToNinja(x);
  });
  const variables = [ [ 'framework', framework ],
                      [ 'copy_headers',
                        copyHeaders.map((x) => this.gypPathToNinja(x)) ] ];
  this.ninja.build(output, 'compile_ios_framework_headers', all_headers, {
    variables: variables,
    orderOnly: prebuild
  }).forEach(x => outputs.push(x));
};

NinjaWriter.prototype.writeMacBundleResources =
    function writeMacBundleResource(resources, bundleDepends) {
  /* Writes ninja edges for 'mac_bundle_resources'. */
  const xcassets = [];

  const extraEnv = this.xcodeSettings.getPerTargetSettings();
  let env = this.getSortedXcodeEnv(extraEnv);
  env = this.computeExportEnvString(env);
  const isBinary = this.xcodeSettings.isBinaryOutputFormat(this.configName);

  gyp.xcodeEmulation.getMacBundleResources(
      generatorDefaultVariables['PRODUCT_DIR'],
      this.xcodeSettings, resources.map(x => this.gypPathToNinja(x))
  ).forEach(([ output, res ]) => {
    output = this.expandSpecial(output);
    if (path.extname(output) !== '.xcassets') {
      this.ninja.build(output, 'mac_tool', res, {
        variables: [ [ 'mactool_cmd', 'copy-bundle-resource' ],
                     [ 'env', env ], [ 'binary', isBinary ] ]
      });
      bundleDepends.push(output);
    } else {
      xcassets.push(res);
    }
  });
...
NinjaWriter.prototype.writeMacXCassets =
    function writeMacXCassets(xcassets, bundleDepends) {
  /*
  ...
  const settings = this.xcodeSettings.xcodeSettings[this.configName];
  Object.keys(settingsToArg).forEach((settingsKey) => {
    const argName = settingsToArg[settingsKey];
    const value = settings[settingsKey];
    if (value)
      extraArguments[argName] = value;
  });

  let partialInfoPlist;
  if (Object.keys(extraArguments).length !== 0) {
    const partialInfoPlist = this.gypPathToUniqueOutput(
        'assetcatalog_generated_info.plist');
    extraArguments['output-partial-info-plist'] = partialInfoPlist;
  }

  const outputs = [];
  outputs.push(path.join(
      this.xcodeSettings.getBundleResourcesFolder(),
      'Assets.car'));
  if (partialInfoPlist.length !== 0)
    outputs.push(partialInfoPlist);

  const keys = quoteShellArgument(JSON.stringify(extraArguments), this.flavor);
  const extraEnv = this.xcodeSettings.getPerTargetSettings();
  let env = this.getSortedXcodeEnv(extraEnv);
  env = this.computeExportEnvString(env);

  this.ninja.build(outputs, 'compile_xcassets', xcassets, {
    variables: [ [ 'env', env ], [ 'keys', keys ] ]
  }).forEach(x => bundleDepends.push(x));
  return partialInfoPlist;
};

NinjaWriter.prototype.writeMacInfoPlist =
    function writeMacInfoPlist(partialInfoPlist, bundleDepends) {
  /* Write build rules for bundle Info.plist files. */
  let [ infoPlist, out, defines, extraEnv ] =
      gyp.xcodeEmulation.getMacInfoPlist(
          generatorDefaultVariables['PRODUCT_DIR'],
          this.xcodeSettings, this.gypPathToNinja.bind(this));
  if (!infoPlist)
    return;

  out = this.expandSpecial(out);
  if (defines && defines.length) {
    // Create an intermediate file to store preprocessed results.
    const intermediatePlist = this.gypPathToUniqueOutput(
        path.basename(infoPlist));
    defines = defines.map(d => define(d, this.flavor)).join(' ');
    infoPlist = this.ninja.build(
        intermediatePlist, 'preprocess_infoplist', infoPlist, {
  ...
  if (partialInfoPlist) {
    const intermediatePlist = this.gypPathToUniqueOutput(
        'merged_info.plist');
    infoPlist = this.ninja.build(
        intermediatePlist, 'merge_infoplist',
        [ partialInfoPlist, infoPlist ]);
  }

  let keys = this.xcodeSettings.getExtraPlistItems(this.configName);
  keys = quoteShellArgument(JSON.stringify(keys), this.flavor);
  const isBinary = this.xcodeSettings.isBinaryOutputFormat(this.configName);
  this.ninja.build(out, 'copy_infoplist', infoPlist, {
    variables: [ [ 'env', env ], [ 'keys', keys ], [ 'binary', isBinary ] ]
  });
  bundleDepends.push(out);
};

NinjaWriter.prototype.writeSources = function writeSources(ninjaFile,
                                                           configName, config,
                                                           sources, prepends,
                                                           precompiledHeader,
                                                           spec) {
  ...
  if (this.toolset === 'host') {
    this.ninja.variable('ar', '$ar_host');
    this.ninja.variable('cc', '$cc_host');
    this.ninja.variable('cxx', '$cxx_host');
    this.ninja.variable('ld', '$ld_host');
    this.ninja.variable('ldxx', '$ldxx_host');
    this.ninja.variable('nm', '$nm_host');
    this.ninja.variable('readelf', '$readelf_host');
  }

  if (this.flavor !== 'mac' || this.archs.length === 1) {
    return this.writeSourcesForArch(this.ninja, configName, config, sources,
                                    prepends, precompiledHeader, spec);
  } else {
    const res = {};
    this.archs.forEach((arch) => {
      res[arch] = this.writeSourcesForArch(this.archSubninjas[arch], configName,
                                           config, sources, prepends,
                                           precompiledHeaderes, spec, arch);
    });
    return res;
  }
};

NinjaWriter.prototype.writeSourcesForArch = function writeSourcesForArch(
    ninjaFile, configName, config, sources, prepends, precompiledHeader, spec,
    arch) {
  /* Write build rules to compile all of |sources|. */
  ...
  let cflagsObjCC;
  if (this.flavor === 'mac') {
    cflags = this.xcodeSettings.getCflags(configName, arch);
    cflagsC = this.xcodeSettings.getCflagsC(configName, arch);
    cflagsCC = this.xcodeSettings.getCflagsCC(configName, arch);
    cflagsObjC = [ '$cflags_c' ].concat(
        this.xcodeSettings.getCflagsObjC(configName, arch));
    cflagsObjCC = [ '$cflags_cc '].concat(
        this.xcodeSettings.getCflagsObjCC(configName, arch));
  } else if (this.flavor === 'win') {
    asmflags = this.msvsSettings.getAsmflags(configName);
    cflags = this.msvsSettings.getCflags(configName);
    cflagsC = this.msvsSettings.getCflagsC(configName);
    cflagsCC = this.msvsSettings.getCflagsCC(configName);
    extraDefines = this.msvsSettings.getComputedDefines(configName);
    // See comment at cc_command for why there's two .pdb files.
    let pdbpathC;
    let pdbpathCC;
    pdbpathC = pdbpathCC = this.msvsSettings.getCompilerPdbName(
        configName, this.expandSpecial.bind(this));
    if (!pdbpathC) {
      let obj = 'obj';
      if (this.toolset !== 'target')
        obj += '.' + this.toolset;
      const pdbpath = path.normalize(path.join(obj, this.baseDir, this.name));
      pdbpathC = pdbpath + '.c.pdb';
      pdbpathCC = pdbpath + '.cc.pdb';
    }
    this.writeVariableList(ninjaFile, 'pdbname_c', [ pdbpathC ]);
    this.writeVariableList(ninjaFile, 'pdbname_cc', [ pdbpathCC ]);
    this.writeVariableList(ninjaFile, 'pchprefix', [ this.name ]);
  } else {
    cflags = config['cflags'] || [];
    cflagsC = config['cflags_c'] || [];
    cflagsCC = config['cflags_cc'] || [];
  }

  // Respect environment variables related to build, but target-specific
  // flags can still override them.
  if (this.toolset === 'target') {
    cflagsC = process.env['CPPFLAGS'].split(/\s+/).concat(
        process.env['CFLAGS'].split(/\s+/), cflagsC);
    cflagsCC = process.env['CPPFLAGS'].split(/\s+/).concat(
        process.env['CXXFLAGS'].split(/\s+/), cflagsCC);
  } else if (this.toolset === 'host') {
    cflagsC = process.env['CPPFLAGS_host'].split(/\s+/).concat(
        process.env['CFLAGS_host'].split(/\s+/), cflagsC);
    cflagsCC = process.env['CPPFLAGS_host'].split(/\s+/).concat(
        process.env['CXXFLAGS_host'].split(/\s+/), cflagsCC);
  }

  const defines = (config['defines'] || []).concat(extraDefines);
  this.writeVariableList(ninjaFile, 'defines',
                         defines.map(d => define(d, this.flavor)));
  if (this.flavor === 'win') {
    this.writeVariableList(ninjaFile, 'asmflags',
                           asmflags.map(f => this.expandSpecial(f)));

    const t = this.msvsSettings.getRcFlags(configName,
                                           this.gypPathToNinja.bind(this));
    this.writeVariableList(ninjaFile, 'rcflags', t.map((f) => {
      return quoteShellArgument(this.expandSpecial(f), this.flavor);
    }));
  }

  let includeDirs = config['include_dirs'] || [];

  const env = this.getToolchainEnv();
  if (this.flavor === 'win')
    includeDirs = this.msvsSettings.adjustIncludeDirs(includeDirs, configName);

  this.writeVariableList(ninjaFile, 'includes', includeDirs.map((i) => {
    return quoteShellArgument(`-I${this.gypPathToNinja(i, env)}`, this.flavor);
  }));

  if (this.flavor === 'win') {
    let midlIncludeDirs = config['midl_include_dirs'] || [];
    midlIncludeDirs = this.msvsSettings.adjustMidlIncludeDirs(
        midlIncludeDirs, configName);
    this.writeVariableList(ninjaFile, 'midl_includes',
                           midlIncludeDirs.map((i) => {
      return quoteShellArgument(`-I${this.gypPathToNinja(i, env)}`,
                                this.flavor);
    }));
  }

  const pchCommands = precompiledHeader.getPchBuildCommands(arch);
  if (this.flavor === 'mac') {
    // Most targets use no precompiled headers, so only write these if needed.
    [ [ 'c', 'cflags_pch_c' ], [ 'cc', 'cflags_pch_cc' ],
      [ 'm', 'cflags_pch_objc' ], [ 'mm', 'cflags_pch_objcc' ]
    ].forEach(([ ext, v ]) => {
      const include = precompiledHeader.getInclude(ext, arch);
      if (include)
        ninjaFile.variable(v, include);
    });
  }

  const arflags = config['arflags'] || [];

  this.writeVariableList(ninjaFile, 'cflags',
                         cflags.map(f => this.expandSpecial(f)));
  this.writeVariableList(ninjaFile, 'cflags_c',
                         cflagsC.map(f => this.expandSpecial(f)));
  this.writeVariableList(ninjaFile, 'cflags_cc',
                         cflagsCC.map(f => this.expandSpecial(f)));
  if (this.flavor === 'mac') {
    this.writeVariableList(ninjaFile, 'cflags_objc',
                           cflagsObjC.map(f => this.expandSpecial(f)));
    this.writeVariableList(ninjaFile, 'cflags_objcc',
                           cflagsObjCC.map(f => this.expandSpecial(f)));
  }
  this.writeVariableList(ninjaFile, 'arflags',
                         arflags.map(f => this.expandSpecial(f)));
  ninjaFile.newline();
  const outputs = [];
  let hasRCSource = false;
  sources.forEach((source) => {
    let [ filename, ext ] = gyp.common.splitext(source);
    ext = ext.slice(1);
    let objExt = this.objExt;

    let command;
    if (ext === 'cc' || ext === 'cpp' || ext === 'cxx') {
      command = 'cxx';
      this.usesCpp = true;
    } else if (ext === 'c' || (ext === 'S' && this.flavor !== 'win')) {
      command = 'cc';
    } else if (ext === 's' && this.flavor !== 'win') {
      // Doesn't generate .o.d files.
      command = 'cc_s';
    } else if (this.flavor === 'win' && ext === 'asm' &&
               !this.msvsSettings.hasExplicitAsmRules(spec)) {
      command = 'asm';
      // Add the _asm suffix as msvs is capable of handling .cc and
      // .asm files of the same name without collision.
      objExt = '_asm.obj';
    } else if (this.flavor === 'mac' && ext === 'm') {
      command = 'objc';
    } else if (this.flavor === 'mac' && ext === 'mm') {
      command = 'objcxx';
      this.usesCpp = true;
    } else if (this.flavor === 'win' && ext === 'rc') {
      command = 'rc';
      objExt = '.res';
      hasRCSource = true;
    } else {
    ...
    const input = this.gypPathToNinja(source);
    let output = this.gypPathToUniqueOutput(filename + objExt);
    if (arch)
      output = addArch(output, arch);
    let implicit = precompiledHeader.getObjDependencies([ input ], [ output ],
                                                        arch);
    let variables = [];
    if (this.flavor === 'win') {
      [ variables, output, implicit ] = precompiledHeader
          .getFlagsModifications(input, output, implicit, command, cflagsC,
                                 cflagsCC, this.expandSpecial.bind(this));
    }
    ninjaFile.build(output, command, input, {
      implicit: implicit.map(([ _1, _2, gch]) => gch),
      orderOnly: prepends,
      variables: variables
    });
    outputs.push(output);
  ...
  if (hasRCSource) {
    const resourceIncludeDirs = config['resource_include_dirs'] || includeDirs;
    const t = resourceIncludeDirs.map((i) => {
      return quoteShellArgument(`-I${this.gypPathToNinja(i, env)}`,
                                this.flavor);
    });
    this.writeVariableList(ninjaFile, 'resource_includes', t);
  }

  this.writePchTargets(ninja, pchCommands);
