...
NinjaMain.prototype.rulesAndTargets = function rulesAndTargets() {
  ...
  const ninjaList = this.targetList.map((target) => {
    const ninja = new Ninja(
        this.configDir, target,
...
NinjaMain.prototype.defaults = function defaults() {
  ...
  this.params.build_files.forEach((buildFile) => {
    this.targetList.forEach((target) => {
      ...
      defaults.add(ninjas[target].output());
      (targetDicts[target].dependencies || []).forEach((dep) => {
        defaults.add(ninjas[dep].output());
      });
    });
  });
