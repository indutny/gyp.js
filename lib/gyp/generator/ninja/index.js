...
function Ninja(options) {
  ...
  const buildFile = parsed.buildFile;
  ...
  this.objDir = path.join(this.outDir, obj, this.intDir);
...
Ninja.prototype.generate = function generate() {
  ...
  if (rule) {
    let implib;
    if (this.flavor === 'win32' && rule === 'solink')
      implib = `/IMPLIB:${out[0]}.lib`;
...
function NinjaMain(targetList, targetDicts, data, params, config) {
  ...
  this.mainTarget = this.targetDicts[this.targetList[0]].configurations[config];
  const parsed = gyp.common.parseQualifiedTarget(this.targetList[0]);
  this.mainDir = path.dirname(parsed.buildFile);
...
NinjaMain.prototype.makeGlobalValue = function makeGlobalValue(value) {
  if (path.isAbsolute(value))
    ...
    return path.join(this.mainDir, value);
};
...
NinjaMain.prototype.vars = function vars() {
  ...
  const makeGlobal = this.mainTarget.make_global_settings || [];
