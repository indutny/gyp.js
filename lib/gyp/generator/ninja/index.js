...
Ninja.prototype.vars = function vars() {
  ...
  if (targetDict.xcode_settings)
    cflags = cflags.concat(targetDict.xcode_settings.WARNING_CFLAGS || []);
  ...
  actions.forEach((action) => {
    ...
    this.n.rule(actionRule, {
      command: this.n.escape(`cd ${base} && ` +
               `${action.action.map(c => this.expand(c)).join(' ')}`)
    });
...
Ninja.prototype.generate = function generate() {
  ...
  (targetDict.dependencies || []).forEach((dep) => {
    ...
    if (depType === 'static_library')
      objs = objs.concat(depOut);
    deps = deps.concat(depOut);
  });
  ...
  (targetDict.sources || []).forEach((source) => {
    ...
    const obj = gyp.common.cachedRelative(
        this.outDir,
        this.objDir + '/' + path.dirname(source) + '/' + objBasename);
  ...
  if (rule) {
    this.n.build(rule, [ out[0] ], objs.filter(filterLinkable), {
      orderOnlyDeps: objs.filter(filterNotLinkable).concat(deps)
    });
  }
...
NinjaMain.prototype.rulesAndTargets = function rulesAndTargets() {
  ...
  const ninjaList = this.targetList.map((target, index) => {
    const ninja = new Ninja(
        index, this.configDir, target,
        this.targetDicts[target].configurations[this.config],
        ninjas, this.config);
    ninjas[target] = ninja;
    return ninja;
  });
...
NinjaMain.prototype.defaults = function defaults() {
  ...
  main.def('all', Array.from(defaults));
