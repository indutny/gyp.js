...
NinjaMain.prototype.vars = function vars() {
  ...
  if (process.platform === 'darwin') {
    ...
    const cl = gyp.platform.win.genEnvFiles(this.outDir,
                                            this.options.generator_flags);

    // TODO(indutny): clang on windows? No way?!
    main.declare('cl_x86', cl.x86);
    main.declare('cl_x64', cl.x64);
    main.declare('ml_x86', 'ml.exe');
    main.declare('ml_x64', 'ml64.exe');
    main.declare('mt', 'mt.exe');

    // TODO(indutny): arch should be configurable
    main.declare('arch', 'environment.x64');
    cc = '$cl_x64';
    cxx = '$cl_x64';
    main.declare('asm', '$ml_x64');
  } else {
    cc = 'gcc';
    cxx = 'g++';
  }
...
NinjaMain.prototype.rulesAndTargets = function rulesAndTargets() {
  ...
  if (process.platform === 'win32') {
    ...
    gyp.platform.darwin.ninjaRules(main, useCxx);
  }
