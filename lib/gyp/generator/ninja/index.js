...
Ninja.prototype.vars = function vars() {
  ...
  // OSX uses xcode_settings
  if (this.flavor !== 'darwin') {
  ...
  if (cflags_c.length !== 0)
    this.n.declare('cflags_c', this.n.escape(cflags_c.join(' ').trim()));
  if (cflags_cc.length !== 0)
    this.n.declare('cflags_cc', this.n.escape(cflags_cc.join(' ').trim()));
...
Ninja.prototype.generate = function generate() {
  ...
  if (/icuuc/.test(targetDict.target_name)) {
  ...
  (targetDict.dependencies || []).forEach((dep) => {
    ...
    if (depType === 'static_library')
      objs = objs.concat(depOut.filter(filterLinkable));
