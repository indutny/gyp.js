...
Ninja.prototype.srcPath = function srcPath(p) {
  ...
  return path.relative(this.outDir,
                       path.normalize(path.join(this.srcDir, p)));
};
...
Ninja.prototype.generate = function generate(data, params) {
  ...
  if (cflags.length !== 0)
    this.n.declare('cflags', this.n.escape(cflags.join(' ')));
  ...
  (targetDict.sources || []).forEach((source) => {
    ...
    const obj = path.relative(
        this.outDir,
        path.join(this.objDir, path.dirname(source), objBasename));
...
function generateConfigOutput(targetList, targetDicts, data, params, config) {
  ...
  main.rule('link', {
    command: `$${useCxx ? 'ldxx' : 'ld'} ` +
             `$${useCxx ? 'ldflags_cc' : 'ldflags_c'} $in -o $out`,
    pool: 'link_pool',
