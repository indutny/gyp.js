  ...
  return function(name, options) {
    ...
    const append = function(value) {
      if (value !== undefined) {
        ...
        if (value !== undefined && options.prefix) value = options.prefix + value;
        if (value) flags.push(value);
      }
    };
...
function compilerFlags(compiler) {
  ...
  if (compiler) {
    ...
    cl('Optimization', { map: {'0': 'd', '1': '1', '2': '2', '3': 'x'}, prefix: '/O', defvalue: '2' });
    ...
    cl('FloatingPointModel', { map: {'0': 'precise', '1': 'strict', '2': 'fast'}, prefix: '/fp:', defvalue: '0' })
    ...
    if (true) cflags.push('/FS'); //TODO: (vs_version.short_name in ('2013', '2013e', '2015'))
...
function librarianFlags(librarian) {
    ...
    if (librarian) {
      ...
      //TODO: libflags.extend(self._GetAdditionalLibraryDirectories('VCLibrarianTool', config, gyp_to_build_path))
...
function linkerFlags(linker) {
  ...
  if (linker) {
    ...
    //TODO: ldflags.extend(self._GetAdditionalLibraryDirectories(linker, config, gyp_to_build_path));
    ld('DelayLoadDLLs', { prefix: '/DELAYLOAD:' })
    ld('TreatLinkerWarningAsErrors', { prefix: '/WX', map: {'true': '', 'false': ':NO'} });
    ld('OutputFile', { prefix: '/OUT:', map: expandMSBuildMacros });
    if (linker.GenerateDebugInformation) {
      ld('ProgramDatabaseFile', { prefix: '/PDB:', map: expandMSBuildMacros });
    }
    ld('ProfileGuidedDatabase', { prefix: '/PGD:', map: expandMSBuildMacros });
    ...
    if (minimum_required_version) minimum_required_version = ',' + minimum_required_version;
    ld('SubSystem', { map: {'1': 'CONSOLE' + minimum_required_version, '2': 'WINDOWS' + minimum_required_version }, prefix: '/SUBSYSTEM:' });
    ...
    ld('ImageHasSafeExceptionHandlers', { map: {'false': ':NO', 'true': ''}, prefix: '/SAFESEH', 
    ...
    if (!ldflags.find(f => f.startsWith('/DYNAMICBASE') || f.startsWith('/FIXED')))
