...
VisualStudioVersion.prototype._setupScriptInternal =
  ...
  const sdkDir = process.environment['WindowsSDKDir'];
  let setupPath;
  if (sdkDir)
    setupPath = path.normalize(path.join(sdkDir, 'Bin/SetEnv.Cmd'));
  if (this.sdkBased && sdkDir && fs.existsSync(setupPath))
  ...
  if (targetArch === 'x86') {
    if (this.shortName >= '2013' && !/e$/.test(this.shortName) &&
      ...
      return [ path.normalize(
         path.join(this.path, 'VC/vcvarsall.bat')), 'amd64_x86' ];
    }

    // Otherwise, the standard x86 compiler.
    return [ path.normalize(
      path.join(this.path, 'Common7/Tools/vsvars32.bat')) ];
  } else {
    let arg = 'x86_amd64';
    ...
    return [ path.normalize(
        path.join(this.path, 'VC/vcvarsall.bat')), arg ];
  }
}

VisualStudioVersion.prototype.setupScript = function setupScript(target_arch) {
...
function getVSVersion(flags) {
  if (vsVersion)
    return vsVersion;
  vsVersion = selectVisualStudioVersion(flags['msvs_version'] || 'auto');
  return vsVersion;
}
