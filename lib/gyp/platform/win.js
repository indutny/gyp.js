...
win.ninjaRules = function ninjaRules(n, outDir, vars) {
  let envFile = win.genEnvironment(outDir,
                                   vars['MSVS_VERSION'] || 'auto',
                                   vars['target_arch'] || 'ia32');
  if (envFile)
    envFile = ` -e${envFile} `;
  else
    envFile = '';

  const ninjaWrap = `ninja -t msvc -e ${envFile}--`;
  ...
  n.rule('asm', {
    command: `${ninjaWrap} $asm @$out.rsp /c /Fo $out $in`,
    rspfile: '$out.rsp',
  ...
  n.rule('solink', {
    command: `${ninjaWrap} $ld /nologo /DLL /OUT:$out @$out.rsp`,
    rspfile: '$out.rsp',
    rspfile_content: '$in_newline $libs $ldflags',
    pool: 'link_pool',
...
win.adjustLibraries = function adjustLibraries(libs) {
  return libs.map(lib => {
    ...
    if (!lib.toLowerCase().endsWith(suffix))
...
win.getMSVSVersion = function getMSVSVersion(version = 'auto') {
...
win.genEnvironment = function genEnvironment(outDir, version, arch) {
  ...
  let lines;
  try {
    lines = execSync(`${vcvars} & set`, { env: {} }).toString().split(/\r\n/g);
  } catch (e) {
