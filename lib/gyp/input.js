...
function loadBuildFileIncludesIntoDict(subdict, subdictPath, data, auxData,
  ...
  Object.keys(subdict).forEach((k) => {
    const v = subdict[k];
    if (Array.isArray(v)) {
      loadBuildFileIncludesIntoList(v, subdictPath, data, auxData);
    } else if (typeof v === 'object') {
      loadBuildFileIncludesIntoDict(v, subdictPath, data, auxData);
    }
  });
}
...
function loadBuildFileIncludesIntoList(sublist, sublistPath, data, auxData) {
  sublist.forEach((item) => {
    if (Array.isArray(item)) {
      loadBuildFileIncludesIntoList(item, sublistPath, data, auxData);
    } else if (typeof item === 'object') {
      loadBuildFileIncludesIntoDict(item, sublistPath, data, auxData);
    }
  });
}
...
function loadTargetBuildFile(options) {
  ...
  const buildFileData = loadOneBuildFile({
    ...
    if (fileList) {
      ...
      if (cachedValue === undefined) {
        ...
        if (commandString) {
          ...
          try {
            if (useShell) {
              p = gyp.bindings.execSync(contents);
            } else {
...
function load(options) {
  ...
  let targets = buildTargetsDict(data);
  ...
  const depList = buildDependencyList(targets);
  ...
  if (rootTargets && rootTargets.length !== 0) {
    ...
    const pruneRes = pruneUnwantedTargets(
