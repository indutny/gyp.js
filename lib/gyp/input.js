...
function loadTargetBuildFile(buildFilePath, data, auxData, variables, includes,
  ...
  if (depth) {
    ...
    const d = path.relative(depth, path.dirname(buildFilePath));
    if (d === '')
      variables['DEPTH'] = '.';
    else
...
function processVariablesAndConditionsInDict(theDict, phase, variablesIn,
  ...
  let variables = JSON.parse(JSON.stringify(variablesIn));
...
function mergeConfigWithInheritance(newConfigurationDict, buildFile,
  ...
  // Drop abstract.
  if (newConfigurationDict['abstract'])
    delete newConfigurationDict['abstract'];
}

function setUpConfigurations(tagret, targetDict) {
  ...
  if (!targetDict['configurations'])
    targetDict['configurations'] = { 'Default': {} };
  if (!targetDict['default_configuration']) {
    ...
    targetDict['defaultConfiguration'] = concrete.sort()[0];
  }
  ...
  Object.keys(configs).forEach((configuration) => {
    ...
    Object.keys(targetDict).forEach((key) => {
      ...
      // PORT: gyp.simple_copy.deepcopy
      if (!nonConfigurationKeys[keyBase])
  ...
  lists.forEach((listKey) => {
    ...
    if (theDict[regexKey]) {
      theDict[regexKey].forEach((regexItem) => {
        const [ action, pattern ] = regexItem;
        const patternRe = new RegExp(patternRe);
...
function validateRulesInTarget(target, targetDict, extraSourcesForRules) {
  ...
  rules.forEach((rule) => {
    ...
    if (ruleExtensions[ruleExtension]) {
    ...
    sourceKeys.forEach((sourceKey) => {
      (targetDict[sourceKey] || []).forEach((source) => {
        ...
        const sourceRoot = source.slice(0, -sourceExtension.length);
  ...
  data['target_build_files'].forEach((buildFile) => {
    ...
    data[buildFile]['targets'].forEach((target) => {
      const qualifiedName = gyp.common.qualifiedTarget(buildFile,
                                                       target['target_name'],
                                                       target['toolset']);
      if (wantedTargets[qualifiedName])
...
function verifyNoCollidingTargets(targets) {
  ...
  targets.forEach((target) => {
    ...
    const [ _, tpath, name ] = target.match(/^(.*):([^:*])$/);
...
function load(buildFiles, variables, includes, depth, generatorInputInfo, check,
              circularCheck, duplicateBasenameCheck, parallel, rootTargets) {
  ...
  buildFiles.forEach((buildFile) => {
    try {
      ...
      e.message += ` while trying to load ${buildFile}`;
      // TODO(indutny): verify that it does not overwrite `e.stack`
      throw e;
    }
  });
  ...
  let [dependencyNodes, flatList] = buildDependencyList(targets);

  if (rootTargets && rootTargets.length !== 0) {
    // Remove, from |targets| and |flat_list|, the targets that are not deep
    // dependencies of the targets specified in |root_targets|.
    [ targets, flatList ] = pruneUnwantedTargets(
  ...
  [ 'all_dependent_settings',
    'direct_dependent_settins',
    'link_settins' ].forEach((settingsType) => {
    doDependentSettings(settingsType, flatList, targets, dependencyNodes);

    // Take out the dependent settings now that they've been published to all
    // of the targets that require them.
    flatList.forEach((target) => {
      if (targets[target][settingsType])
