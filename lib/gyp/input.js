...
function buildTargetsDict(data) {
  ...
  const targets = {};
  for (let i = 0; i < data['target_build_files'].length; i++) {
    const buildFile = data['target_build_files'][i];
    const t = data[buildFile]['targets'] || [];
...
function qualifyDependencies(targets) {
  ...
  for (let i = 0; i < dependencySections.length; i++) {
    const dep = dependencySections[i];
  ...
  Object.keys(targets).forEach((target) => {
    ...
    const allDependencySections = [];
    for (let i = 0; i < allDependencySections.length; i++) {
      const dependencyKey = allDependencySections[i];
...
function expandWildcardDependencies(targets, data) {
  ...
  Object.keys(targets).forEach((target) => {
    ...
    for (let i = 0; i < dependencySections.length; i++) {
      const dependencyKey = dependencySections[i];
      ...
      while (index < dependencies.length) {
        ...
        for (let i = 0; i < dependencyTargetDicts.length; i++) {
          const dependencyTargetDict = dependencyTargetDicts[i];
          if (dependency_target_dict['suppress_wildcard'])
            continue;
