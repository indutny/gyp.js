...
gyp.ninjaSyntax = require('./gyp/ninja-syntax');
...
gyp.main = function main(args) {
  ...
  parser.addOption('format', 'f', { dest: 'formats', action: 'append',
                   envName: 'GYP_GENERATORS', regenerate: false,
  ...
  if (!options.formats) {
    ...
    if (generateFormats)
      generateFormats = generateFormats.split(/[\s,]/g);
    if (generateFormats) {
      options.formats = generateFormats;
    } else {
      // TODO(indutny): just ninja everywhere
  ...
  for (let i = 0; i < options.formats.length; i++) {
    ...
    const [ generator, flatList, targets, data ] = load(buildFiles, format,
